...
class SupportingTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.task = Task.objects.create(date=date.today(), time_worked=120, description="abc",
                                       project=cls.project, user=cls.test_user1)

    @classmethod
    def tearDownClass(cls):
        for elem in [cls.task, cls.project, cls.test_user1, cls.department, cls.company]:
            elem.delete()

    def test_write_csv_file(self):
        with open("data.csv", "w") as file:
            write_csv_file({self.test_user1: [self.task]}, file)
        self.assertTrue(os.path.isfile("data.csv"))
        data = ["First name;Last name;Date;Worked time;Name project;Description", "abc;abc;2021-06-01;120;abc;abc"]
    ...
    def test_write_xlsx_file(self):
        write_xlsx_file({self.test_user1: [self.task]}, "data.xlsx")
        ...
        self.assertEqual(excel_data_df["Date"].tolist()[0], "2021-06-01")
